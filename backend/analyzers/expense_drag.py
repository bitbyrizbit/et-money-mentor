def compute_expense_drag(funds: list[dict], years: int = 20) -> dict:
    total_invested = sum(f["value"] for f in funds if f.get("value", 0) > 0)
    if not total_invested:
        return {
            "weighted_expense_ratio": 0, "total_invested": 0,
            "gross_corpus_20yr": 0, "net_corpus_20yr": 0,
            "expense_drag_20yr": 0, "switching_to_direct_gain": 0, "annual_drag": 0,
        }

    weighted_expense = sum(
        f["value"] * f.get("expense_ratio", 1.0)
        for f in funds if f.get("value", 0) > 0
    ) / total_invested
    ...
    return {
        ...
        "switching_to_direct_gain": round(max(0, switching_gain), 2),
